...
<span type="info">
This package requires React 19 and depends on `swr` for data fetching and caching.
</span>
...
| Hook | Description |
|---|---|
| `useAddressForm` | Billing and shipping address form state, and saving it to the order |
| `useAvailability` | Fetch availability for a SKU code or ID |
| `useCustomer` | Fetch a customer with their addresses, payment sources and orders |
| `useGiftCards` | Fetch, create and update gift cards |
| `useInStockSubscriptions` | Subscribe a customer to a SKU coming back in stock |
| `useLineItems` | Fetch the order's line items, and update or delete them |
| `useOrder` | Fetch, create and update an order |
| `usePrices` | Fetch, retrieve, and update prices |
| `useShipments` | Fetch the order's shipments and set a shipping method |
| `useSkuList` | Retrieve a single SKU list by ID |
