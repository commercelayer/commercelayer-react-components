...
From the example above, we can replace the cart-recap gray block at the bottom of the page, with the `<HostedCart type="mini" openAdd />` component to open a mini cart every time a new item is added.
