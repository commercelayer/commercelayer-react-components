...
export function formCleaner(address: CombinedAddressType): CombinedAddressType {
  ...
  Object.keys(address).forEach((key) => {
    const keyCleaned = key.replace("shipping_address_", "").replace("billing_address_", "")
    const isNotCleaned = key.startsWith("shipping_address_") || key.startsWith("billing_address_")
    if (isNotCleaned) {
