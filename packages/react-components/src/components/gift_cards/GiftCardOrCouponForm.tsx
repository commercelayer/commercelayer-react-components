...
export function GiftCardOrCouponForm(props: Props): JSX.Element | null {
  ...
  useEffect(() => {
    if (type != null && values[type]?.value === "" && errors != null && errors.length > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault()
    const code = type != null && values[type] != null ? values[type].value : undefined
  ...
  return (order?.gift_card_code && order?.coupon_code) || order == null ? null : (
    <CouponAndGiftCardFormContext.Provider value={{ validation, codeType: type }}>
      <form
        ref={ref}
