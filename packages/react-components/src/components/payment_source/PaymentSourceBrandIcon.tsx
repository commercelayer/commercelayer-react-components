...
export function PaymentSourceBrandIcon({ src, width = 32, children, ...p }: Props): JSX.Element {
  const { brand, issuer_type: issuerType } = useContext(PaymentSourceContext)
  const { brand: customerCardBrand, issuer_type: customerIssuerType } = useContext(
    CustomerPaymentSourceContext
  )
  const cardBrand = brand ?? customerCardBrand ?? issuerType ?? customerIssuerType
