...
export function PaymentSourceDetail({ type, children, ...p }: Props): JSX.Element {
  ...
  const text =
    type in cardObj && cardObj[type] != null ? cardObj[type] : type === "last4" ? "****" : "**"
  const parentProps = {
    type,
    text,
    ...p,
  }
  return children ? <Parent {...parentProps}>{children}</Parent> : <span {...p}>{text}</span>
}
