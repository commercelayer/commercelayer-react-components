import type { Customer, LineItem, Parcel, ParcelLineItem, Sku } from "@commercelayer/sdk"
import { type Context, type JSX, useContext } from "react"
...
export type TGenericChildrenProps<E extends TResources[keyof TResources]> = Omit<
  Props<E>,
  "children" | "attribute" | "context" | "tagElement"
> & {
...
export default function GenericFieldComponent<R extends keyof TResources>(
  ...
  let attributeValue = ""
  const keysContext = Object.keys(resourceContext).filter((key) => key === resource) as [
    keyof ResourceContext[keyof ResourceContext],
  ]
  ...
  if (Tag === "img" && !children) {
    ...
    return (
      <img
        alt={name}
        src={src}
        {...(p as Omit<JSX.IntrinsicElements["img"], "ref" | "children">)}
      />
    )
  }
  const parentProps = {
