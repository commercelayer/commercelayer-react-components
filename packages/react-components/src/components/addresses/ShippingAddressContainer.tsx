...
export function ShippingAddressContainer(props: Props): JSX.Element {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: We want to trigger this effect only on order and config change, not on setCloneAddress or dispatch change
  useEffect(() => {
    if (order && config) {
      setShippingCustomerAddressId({
        dispatch,
