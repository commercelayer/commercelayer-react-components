      ...
      createElement(
        AvailabilityContext.Provider,
        {
          value: {
            ...
            min: { hours: 24, days: 1 },
            max: { hours: 72, days: 3 },
            shipping_method: { name: 'Standard', formatted_price_amount: '$5.00', id: '1', type: 'shipping_methods' } as any
          }
        },
    ...
    render(
      <SkuChildrenContext.Provider value={{ sku: { code: 'SKU001' } as any }}>
        <SkuField attribute='code'>
          {({ attributeValue }) => (
            <span data-testid='custom-field'>{String(attributeValue)}</span>
          )}
        </SkuField>
...
describe('Parent component', () => {
  ...
  it('renders children when provided', () => {
    const Child = ({ label }: { label: string }) => (
      <span data-testid='parent-child'>{label}</span>
    )
