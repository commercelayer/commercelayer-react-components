...
import { EMPTY, getSnapshot, registerSku, subscribe, unregisterSku } from "./pricesBatchStore"

describe("pricesBatchStore", () => {
  afterEach(() => {
    vi.useRealTimers()
  })
  ...
  it("unregisterSku removes a code (no refetch)", async () => {
    ...
    // snapshot is not updated on unregister (cached prices stay)
