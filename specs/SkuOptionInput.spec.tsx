...
test('<SkuOptionInput check type required />', () => {
  ...
  expect(console.error.mock.calls[0][0]).toEqual(
    expect.stringContaining(
      'Warning: Failed prop type: The prop `name` is marked as required in `SkuOptionInput`, but its value is `undefined`'
    )
  )
  expect(console.error.mock.calls[1][0]).toEqual(
    expect.stringContaining(
      'Warning: Failed prop type: The prop `type` is marked as required in `SkuOptionInput`, but its value is `undefined`'
    )
  )
})
